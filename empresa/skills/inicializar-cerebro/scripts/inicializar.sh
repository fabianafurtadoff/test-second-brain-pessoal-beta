...
OLD_AGENT_DIR="agentes/aria"
NEW_AGENT_DIR="agentes/$AGENTE_SLUG"

# Como os placeholders já foram substituídos no find acima, a pasta continua com o nome literal aria
# até a gente renomear
if [ -d "agentes/aria" ]; then
  git mv "agentes/aria" "agentes/$AGENTE_SLUG" 2>/dev/null || mv "agentes/aria" "agentes/$AGENTE_SLUG"
fi
